...
export const secretFactory = (secretKey?: string | null): SecretEncoder => {
...
const dataToString = (data: EncodedData) => {
	return `${Buffer.from(data.iv).toString('base64')}:${Buffer.from(data.content).toString('base64')}`
}

const stringToData = (data: string): EncodedData => {
	const [iv, content] = data.split(':').map(data => Buffer.from(data, 'base64').toString('utf-8'))
	return { iv, content }
}

const SECRET_KEY = '_secret__'

export const secretStorageFactory = (file: string, { decrypt, encrypt }: SecretEncoder) => {
	let entries: Record<string, string> | undefined
	const getFileContents = (): Promise<Record<string, string>> => {
		if (entries) {
			return Promise.resolve(entries)
		}
		return readFile(file, 'utf-8').then(text => text.split('\n').map(d => d.trim()).map(line => {
	...
	const save = async (data: Record<string, string>) => {
		...
		return writeFile(file, Object.entries(entries).map(([key, value]) => `${key}=${value}`).join('\n'), 'utf-8')
	}
	return {
		...
		getAll: async () => {
			const ent = await getFileContents()
			return Object.fromEntries(Object.entries(ent).map(([key, value]) => {
				if (value && value.indexOf(SECRET_KEY) == 0) {
					const secretData = value.substring(SECRET_KEY.length)
					const data = stringToData(secretData)
